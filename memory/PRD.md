...
### Iteration 75 · SuperAdmin: single-user progress + accurate enrollment analytics + filter cleanup — Jun 2026
Verified by testing_agent (`/app/test_reports/iteration_75.json`): backend 7/7 pytest, frontend 100%. No blocking issues.
- **Single-user progress tracking (bug)**: the `/api/admin/user360/{id}` endpoint + `User360Drawer` existed but the **Users tab had no way to open them**. Wired it in (`UserControlRow.jsx`: clickable name `user-progress-<id>` + a 360° icon button `view-360-<id>`; `SuperAdminPortal.jsx` `UserTable` now passes `onOpenUser=setOpenUserId`). Clicking a user opens the Learner-360 drawer showing enrollments+progress%, credentials, quiz/assessment attempts, AI sessions, recent logins, referrals, audit trail. Also added an **error+retry state** to both `User360Drawer` and `Org360Drawer` (were stuck on "Loading…" forever on a failed fetch).
- **Inaccurate enrollment analytics (bug: "80 enrolments / 36 users")**: root cause = courses were seeded with **fake marketing `enrolled_count`** values (18,420 / 24,310 …) which the enroll endpoint then `$inc`-ed. New `content_fixups.reconcile_enrollments()` runs LAST in `seed_database()` on every boot: de-dupes enrollment docs, rebuilds the unique `(user_id, course_id)` index safely (so it can't abort startup on legacy dupes), and **recomputes every course's `enrolled_count` to the REAL enrollment total**. Enroll endpoint confirmed idempotent (double-POST → +1 only). Result: no course can ever report more enrollments than real records. NOTE: this removes the inflated marketing numbers from public course cards (now show real counts) — this is the "accurate" behaviour the superadmin asked for; can be reverted if aspirational numbers are wanted.
- **Catalogue filter cleanup (user chose option b)**: `/api/catalog/industries` and new `/api/catalog/difficulties` now derive from live course data and **drop empty options** (Industry: Real Estate/Construction/Aviation gone; Difficulty: only Intermediate/Advanced/Architect/Enterprise Leader remain — Fundamental/Beginner/Expert/CXO gone). Each option shows a course count. `CourseCatalog.jsx` fetches both + renders counts.
- **⚠️ REDEPLOY REQUIRED**: all fixes are in preview/code — production (ithr.online) only reflects them after a redeploy (the reconcile hook then self-corrects the production DB automatically).
...
- **User escalation (4th time)**: "the course catalogue does not have the courses in each of its categories." Diagnosed TWO root causes:
  ...
  2. **3 empty categories** ("AI Fundamentals", "Generative AI", "AI Agents") were shown in the sidebar (hardcoded `CATEGORIES` list of 21) but had zero courses → clicking them showed "No courses match your filters."
- **Fixes (all code/data — ZERO LLM credits)**:
  - **Baked generated content into the repo**: exported all 17 non-builder courses' full 15-module content (952 lessons, 3.1 MB) from the preview DB to `backend/data/generated_courses.json` (committed). New `backend/seed_generated_courses.py::seed_generated_courses()` upserts this content on every startup (idempotent) — graduates each stub → published 15-module course, cleans the stub description, sets `status="published"`, preserves the course-level `id` (no orphaned enrollments). Wired into `server.py::seed_database()` after the stub loop. **This makes the content reproducible across environments — production gets full content on redeploy.**
  - **`GET /api/catalog/categories` now derives from live data**: aggregates distinct categories present in `courses`, drops empties, keeps the curated `CATEGORIES` ordering, and returns a per-category `counts` map.
  - **Frontend `CourseCatalog.jsx`**: sidebar now shows a course count next to each category (reinforces that every category is populated).
- **Verified in PREVIEW**: `/api/courses` → 28 courses, all `status=published`, all 15 modules. `/api/catalog/categories` → 18 categories (empties gone) + counts. Detail page for previously-stub `generative-ai-executives` → published, 15 modules, clean description. Screenshot of `/courses` confirms all cards show "FULL CURRICULUM". Self-tested (API + screenshot); no testing_agent run (conserving credits per user).
- **⚠️ ACTION REQUIRED — REDEPLOY**: These fixes are in code + committed data. PRODUCTION (ithr.online) will only reflect them AFTER the user redeploys. On the next deploy, `seed_generated_courses()` publishes all 17 baked courses in the production DB automatically.
...
- `FRONTEND_URL=https://enterprise-agent-dev.preview.emergentagent.com` — used for the reset link URL in the email body.
